...
#pragma pack(pop)
    ...
    for (didx = 0; didx < DDR_CFG_T_MAX_DIMMS; ++didx) {
        ...
        if (validate_dimm(&dimm_config_table[didx], 0)) {
            if (ddr_verbose())
                report_dimm(&dimm_config_table[didx], 0, dimm_count);
            ++dimm_count;
        } else {
		break;
...
#define DDR4_tDLLK         768        /* 768 nCK */
    ...
    /* LMC(0)_COMP_CTL2 */
    {
        ...
        comp_ctl2.cn78xx.ntune_offset    = 0;
        comp_ctl2.cn78xx.ptune_offset    = 0;
...
#pragma pack(pop)
...

    {
        ...
        ** ranks 0 and 2, but only 2 ranks are active. */
        ...
        for (rankx = 0; rankx < dimm_count * 4; rankx++) {
                    ...
                    if ((ddr_interface_bytemask & 0xff) == 0xff) {
                        if (save_ecc_ena) {
                            ...
                            if ((save_byte8 != lmc_wlevel_rank.cn78xx.byte3) &&
                                ...
                                old_byte8 = ~1 & divide_nint(lmc_wlevel_rank.cn78xx.byte3
                                                             + lmc_wlevel_rank.cn78xx.byte4
                                                             + 2 /* round-up*/ , 2);
            ...
            ddr_print("Rank(%d) Wlevel Rank %#5x, 0x%016llX : %2d,%2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %s\n",
                      rankx,
                      lmc_wlevel_rank.cn78xx.status,
                      lmc_wlevel_rank.u64,
                      lmc_wlevel_rank.cn78xx.byte8, old_byte8, wl_status_strings[byte_test_status[8]],
...
#ifdef CAVIUM_ONLY
    {
	    ...
	    SET_DDR_DLL_CTL3(dll90_byte_sel, ENCODE_DLL90_BYTE_SEL(i));
            cvmx_write_csr_node(node, CVMX_LMCX_DLL_CTL3(ddr_interface_num),	ddr_dll_ctl3.u64);
            cvmx_read_csr_node(node, CVMX_LMCX_DLL_CTL3(ddr_interface_num));
            ddr_dll_ctl3.u64 = cvmx_read_csr_node(node, CVMX_LMCX_DLL_CTL3(ddr_interface_num));
        ...
        //cvmx_write_csr_node(node, CVMX_LMCX_DLL_CTL3(ddr_interface_num),	save_ddr_dll_ctl3.u64);
    }
#endif  /* CAVIUM_ONLY */
    ...
    if ((s = lookup_env_parameter("ddr_auto_set_dll_offset")) != NULL) {
        ...
        limit_l2_ways(0, 0);
#endif
        auto_set_dll_offset( 1 /* 1=write */, node, ddr_interface_num, cpu_id,
			     ddr_interface_64b);
        auto_set_dll_offset( 2 /* 2=read */, node, ddr_interface_num, cpu_id,
			     ddr_interface_64b);
