	...
	if (octeon_has_feature(OCTEON_FEATURE_CIU3)) {
		cvmx_coremask_for_each_node(node, gd->arch.node_mask) {
			if (node) {
				...
				oci_ctl.u64 = cvmx_read_csr_node(node,
								 CVMX_L2C_OCI_CTL);
...
init_fnc_t *init_sequence[] = {
	...
	octeon_setup_clocks,			/* 5 */
#if !defined(CONFIG_OCTEON_SIM_HW_DIFF)
	init_twsi_bus,				/* 6 */
#endif
	board_early_init_f,			/* 7 */
	board_fdt_init_f,			/* 8 */
...
void board_init_r(gd_t * id, u64 dest_addr)
{
	...
	if (!(gd->flags & GD_FLG_MEMORY_PRESERVED)) {
		...
		if (!OCTEON_IS_OCTEON1PLUS()) {
			...
			if (gd->ram_size > MB(512)) {
				debug("%s: Clearing 0x%llx bytes of memory starting at 0x20000000\n",
...
# endif
	...
	dnull_ptr_check("SPI initialized");
#endif
...
#if defined(CONFIG_OCTEON_MMC) && !defined(CONFIG_ENV_IS_IN_MMC)
