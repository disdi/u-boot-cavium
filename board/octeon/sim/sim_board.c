...
int checkboard(void)
{
	...
	/* cn76xx is a subset of cn78xx, check 76xx before checking 78xx */
		...
		/* GSER0,1 =PCIe, GSER4 (DLM) =STATA, GSER5,6 (DLM) = BGX2 */
		octeon_configure_qlm(2, 1250, CVMX_QLM_MODE_SGMII, 0, 0, 2, 0);
		octeon_configure_qlm(3, 3125, CVMX_QLM_MODE_XAUI, 0, 0, 2, 0);
	} else
#endif
	if (OCTEON_IS_MODEL(OCTEON_CN75XX)) {
		/* GSER4: SGMII, GSER5: SGMII/10G-KR */
		int dlm;
		cvmx_gserx_phy_ctl_t phy_ctl;
		cvmx_gserx_cfg_t gserx_cfg = {.u64 = 0};

		//FIXME: provisional code: Enable BGX DLMs

		dlm  = 4;
		gserx_cfg.s.bgx = 1;
		cvmx_write_csr(CVMX_GSERX_CFG(dlm), gserx_cfg.u64);
		cvmx_write_csr(CVMX_GSERX_PHY_CTL(dlm), 0);
