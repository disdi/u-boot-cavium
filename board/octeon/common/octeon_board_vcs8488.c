...
#include <asm/arch/octeon_board_vcs8488.h>
#include <asm/arch/octeon_fdt.h>
#include <asm/arch/octeon_eth.h>

/**
 * This file contains routines common for the Vitesse VCS8488 PHY device
 ...
 */
static int octeon_fdt_vcs8488_config(const struct eth_device *ethdev)
{
	...
	debug("%s: Configuring Vitesse VCS8488 PHY device for %s\n",
...
int octeon_vcs8488_config(void)
{
	...
	do {
		...
		if (oct_eth_info->phy_device_type == VITESSE) {
			if (octeon_fdt_vcs8488_config(ethdev)) {
