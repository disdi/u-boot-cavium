 ...
 */
uint32_t boot_bus_time_encode(int64_t time, uint64_t clock_period, int max)
	__attribute__((weak, alias("__boot_bus_time_encode")));

uint32_t __boot_bus_time_encode(int64_t time, uint64_t clock_period, int max)
{
	uint32_t clocks;
	int clock_mult =
		(OCTEON_IS_OCTEON1() || OCTEON_IS_MODEL(OCTEON_CN58XX)) ? 4 : 8;
	if (time < 0)
		clocks = 0x3f;
	if (time == 0)
		return 0;
...
#define BOOT_BUS_TIME_ENCODE(time) \
	boot_bus_time_encode((time), clock_period, 0x3f)
...
static const struct bb_fdt_prop *config_to_prop(boot_bus_config_t conf)
{
	if (conf < UNKNOWN)
...
static const struct bb_fdt_prop *string_to_prop(const char *str)
{
	int i = 0;
	do {
...
{
	const void *nodep;
	const struct bb_fdt_prop *property;
	int len;
	...
	while (((tag = fdt_next_tag(gd->fdt_blob, node_offset,
		...
		case FDT_PROP:
			...
			if (level == 1) {
				...
				if (!prop) {
					printf("Unknown property for boot bus in device tree: %s\n",
					       prop_str);
				...
				if (len == 4) {
					...
					if ((prop->conf != BUS_WIDTH) &&
					    (prop->conf != PAGES)) {
						clocks = boot_bus_time_encode(value,
									     clock_period,
									     prop->max);
...
# endif
	...
	gd->arch.flash_size = len - 0x400000;
