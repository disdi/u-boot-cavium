...
int checkboard(void)
{
	...
	int iclk_to_use = 0;    /* Reference-clock input to use */
	int host_mode;
	const char *dlm_mode;
	...
	if (!dlm_mode || !strcmp(dlm_mode, "disabled_disabled") ||
		...
		/* DLM0: QSGMII/QSGMII */
		puts("Configuring DLM0 for SGMII/SGMII\n");
		octeon_configure_qlm(0, 5000, CVMX_QLM_MODE_QSGMII_QSGMII, 0, 0,
		...
		octeon_configure_qlm(0, 6250, CVMX_QLM_MODE_RXAUI, 0, 0,
				     2, 2);
	} else {
