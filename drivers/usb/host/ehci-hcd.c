...
static int ehci_td_buffer(struct qTD *td, void *buf, size_t sz)
{
	...
	while (idx < QT_BUFFER_CNT) {
		...
		next = (addr + EHCI_PAGE_SIZE) & ~((dma_addr_t)(EHCI_PAGE_SIZE - 1));
...
#warning CONFIG_SYS_MALLOC_LEN may be too small for EHCI
	 ...
	 */
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(&ctrl->qh_list);
	qh->qh_link = cpu_to_hc32((uint32_t)addr | QH_LINK_TYPE_QH);
	debug("%s: qh_list address is 0x%llx\n", __func__, (u64)addr);
#else
	...
	if (req != NULL) {
		...
		/* Update previous qTD! */
#ifdef CONFIG_OCTEON
		addr = virt_to_phys(&qtd[qtd_counter]);
		debug("%s: Setting req *tdp to 0x%llx, qtd[%d] at 0x%p\n",
		      __func__, (u64)addr, qtd_counter, &qtd[qtd_counter]);
		*tdp = cpu_to_hc32((uint32_t)addr);
#else
	...
	if (length > 0 || req == NULL) {
		...
		do {
			...
			addr = virt_to_phys(&qtd[qtd_counter]);
			*tdp = cpu_to_hc32((uint32_t)addr);
			debug("%s: Setting NULL req tdp: %p, *tdp: 0x%x to 0x%llx, len: %d, &qtd[%d]: %p\n",
			      __func__, tdp, *tdp, (u64)addr, left_length,
			      qtd_counter, &qtd[qtd_counter]);
#else
			*tdp = cpu_to_hc32((uint32_t)&qtd[qtd_counter]);
#endif
			tdp = &qtd[qtd_counter++].qt_next;
...
#ifdef CONFIG_OCTEON
		addr = virt_to_phys(&qtd[qtd_counter]);
		debug("%s: Setting req *tdp to 0x%llx\n", __func__, (u64)addr);
		*tdp = cpu_to_hc32((uint32_t)addr);
#else
		*tdp = cpu_to_hc32((uint32_t)&qtd[qtd_counter]);
#endif
...
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(qh);
	debug("%s: Setting link to 0x%llx\n", __func__, (u64)addr);
	ctrl->qh_list.qh_link = cpu_to_hc32((uint32_t)addr | QH_LINK_TYPE_QH);
#else
...
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(&ctrl->qh_list);
	debug("%s: Setting async queue head ptr to 0x%p (0x%llx)\n", __func__,
	      &ctrl->qh_list, addr);
	ehci_writel(&ctrl->hcor->or_ctrldssegment, (addr >> 16) >> 16);
	ehci_writel(&ctrl->hcor->or_asynclistaddr, addr & 0xffffffff);
#else
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
...
#endif
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(qh_list);
#else
	addr = (dma_addr_t)qh_list;
#endif
	/* Set the high address word (aka segment) for 64-bit controller */
	if (ehci_readl(&ehcic[index].hccr->cr_hccparams) & 1)
		ehci_writel(&ehcic[index].hcor->or_ctrldssegment,
			    (addr >> 16) >> 16);
...
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(qh_list);
	debug("%s: qh_list phys addr=0x%llx\n", __func__, (u64)addr);
	qh_list->qh_link = cpu_to_hc32((uint32_t)addr | QH_LINK_TYPE_QH);
#else
...
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(qh_list);
	ehci_writel(&ehcic[index].hcor->or_ctrldssegment, (addr >> 16) >> 16);
	ehci_writel(&ehcic[index].hcor->or_asynclistaddr, addr & 0xffffffff);
#else
	...
	if (!ehcic[index].periodic_list)
		return -ENOMEM;
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(periodic);
#else
	addr = (uint32_t)periodic;
#endif
	for (i = 0; i < 1024; i++) {
		ehcic[index].periodic_list[i] = (uint32_t)addr
						| QH_LINK_TYPE_QH;
	}
...
#ifdef CONFIG_OCTEON
        addr = virt_to_phys(ehcic[index].periodic_list);
#else
        addr = (uint32_t)ehcic[index].periodic_list;
#endif

	ehci_writel(&ehcic[index].hcor->or_periodiclistbase, addr);
		 ...
		 int elementsize, void *buffer)
{
	...
	for (i = 0; i < queuesize; i++) {
...
#ifdef CONFIG_OCTEON
		addr = virt_to_phys(qh+1);
#else
		addr = (uint32_t)(qh+1);
#endif
		qh->qh_link = (uint32_t)addr | QH_LINK_TYPE_QH;
		if (i == queuesize - 1)
			qh->qh_link = QH_LINK_TERMINATE;

#ifdef CONFIG_OCTEON
		addr = virt_to_phys(td);
#else
		addr = (uint32_t)td;
#endif
		qh->qh_overlay.qt_next = (uint32_t)addr;
		...
		td->qt_token = (elementsize << 16) |
			...
			0x80; /* active */
#ifdef CONFIG_OCTEON
		addr = virt_to_phys((uint8_t *)buffer + i * elementsize);
#else
		addr = (uint32_t)buffer + i * elementsize;
#endif
		td->qt_buffer[0] = (uint32_t)addr;
		...
		td->qt_buffer_hi[0] = (addr >> 16) >> 16;
		td->qt_buffer_hi[1] = td->qt_buffer_hi[0];
		td->qt_buffer_hi[2] = td->qt_buffer_hi[0];
		td->qt_buffer_hi[3] = td->qt_buffer_hi[0];
		td->qt_buffer_hi[4] = td->qt_buffer_hi[0];
