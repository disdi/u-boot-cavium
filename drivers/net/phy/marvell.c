...
/* Marvell 88E1680S */
static int m88e1680s_startup(struct phy_device *phydev)
{
        debug("%s: interface=%d\n", __func__, phydev->interface);
        return 0;
}
/* Marvell 88E1680S */
static int m88e1680s_config(struct phy_device *phydev)
{
	int reg;
	debug("%s: interface=%d\n", __func__, phydev->interface);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1118_PHY_PAGE, 4);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x1b, 0x3fa0);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1118_PHY_PAGE, 0xfd);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x8, 0xb53);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x7, 0x200d);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1118_PHY_PAGE, 0);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x10, 0x3070);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x0, 0x9140);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1118_PHY_PAGE, 4);
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 0x11);
	debug("%s: interface=%d reg 17 %x\n", __func__, phydev->interface, reg);
        return 0;
}


#ifdef CONFIG_PHYLIB_10G
static int m88x3120_config(struct phy_device *phydev)
{
	int reg;
	ulong start_time;
	...
	do {
		reg = phy_read(phydev, 1, MII_BMCR);
		if (reg < 0)
			goto read_error;
	...
	reg = phy_read(phydev, 7, 0);
	if (reg < 0)
		goto read_error;
	reg |= 1 << 12;
	phy_write(phydev, 7, 0, reg);

	/* XNP ability, pause bits, 100BT FD */
	reg = phy_read(phydev, 7, 16);
	if (reg < 0)
		goto read_error;
	reg |= (1 << 12) | (3 < 10) | (1 << 8);
	phy_write(phydev, 7, 16, reg);

	/* 10GB EEE advertisement */
	reg = phy_read(phydev, 7, 60);
	if (reg < 0)
		goto read_error;
	reg |= (1 << 3);
	phy_write(phydev, 7, 60, reg);

	/* EEE legacy, EEE auto, EEE idle count to enter LPI */
	reg = phy_read(phydev, 1, 0xc04e);
	if (reg < 0)
		goto read_error;
	reg |= (1 << 15) | (1 << 14) | 0xff;
	phy_write(phydev, 1, 0xc04e, reg);

	/* Force 100BT duplex, force 100BT, Parallel det en, MDIX control */
	reg = phy_read(phydev, 7, 0xc005);
	if (reg < 0)
		goto read_error;
	reg |= (1 << 7) | (1 << 5) | (1 << 4) | 0x7;
	phy_write(phydev, 7, 0xc005, reg);

	/* Duplex for 100/1000BT, speed advertisement */
	reg = phy_read(phydev, 1, 0xc028);
	if (reg < 0)
		goto read_error;
	reg |= (3 << 8) | 0xF;
	phy_write(phydev, 1, 0xc028, reg);
#endif
	return 0;
...
static struct phy_driver M88E1680_driver = {
	...
	.mask = 0xffffff0,
	.features = PHY_GBIT_FEATURES,
	.config = &m88e1680s_config,
	.startup = &m88e1680s_startup,
	.shutdown = &genphy_shutdown,
};
